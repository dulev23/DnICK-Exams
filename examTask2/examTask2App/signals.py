...
from .models import RealEstate, AgentRealEstate, CharacteristicRealEstate
...
def handle_saving_house(sender, instance, **kwargs):
    all_characteristics = sender.filter(real_estate=instance.real_estate).all()
    if all_characteristics:
        real_estate = all_characteristics[0].real_estate
        real_estate.characteristic = ", ".join(char.characteristic for char in all_characteristics)
        real_estate.save()
